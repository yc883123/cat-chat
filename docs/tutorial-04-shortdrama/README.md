...
批量出片在后台跑，**不挡住你继续聊**。三条盯进度的路：

- 顶栏 **「任务」** 按钮——面板里能看到「运行中 N / 已完成 N」、每批跑到第几段了、以及每批的耗时；
- 聊天区里那一轮工具块会自己滚动刷新，**一段跑完就地变成产物卡片**；
- 不想等了，任务面板里点停止，或者直接跟它说「停」。

成品**挂回消息里直接播放**；真实文件落盘在 `data\generated`：

- 视频按 `{剧名}/{日期}/{单元号}` 分目录（这也是提交前它会问你输出前缀的原因）；
- 想只重跑某几段，改完剧本告诉它段号就行，**不用整集重来**。
