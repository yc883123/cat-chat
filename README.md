...
Cat Chat 是运行在 Windows 本机的通用 AI 自动化工作台。它把在线或本地模型、内置工具、后台任务、Skill、MCP、视觉工具和文件产物统一到一个对话界面中。2.8.1 Beta 让「分支」不再把 Agent 一起锁死——你可以带着当前 Agent 继续，也可以换一个 Agent 再问。
...
- **分支时可选「更换 Agent」（本次新能力）**：点「分支」会先弹一个选择框——**保持当前 Agent**（工具集随历史继承，前缀缓存继续命中）或**更换 Agent**（新会话不固化工具集，回到「发首轮之前」的状态，你可以在输入区重新选 Agent 再问）。选「取消」则什么都不发生。
- **为什么以前分支后就换不了 Agent**：分支会把源会话**已固化的工具集**一起复制到新会话，而工具集一旦非空，Agent 下拉就变成只读——这是为了保住前缀缓存：中途换 Agent 会让整段历史重新计费。本次只在「分支新建会话」这一个时点给出换的机会，**已发过消息的会话仍然不允许中途换 Agent**。
- **「更换 Agent」清掉的是三样绑在一起的东西**：固化的工具集（下拉能解锁的唯一判据）、按旧 Agent 冻结的 Skill 引用、旧的「首轮上下文」折叠卡——最后一项若留着，新会话顶部展示的会是上一任 Agent 的固化上下文，属于误导。**Agent 本身仍然预选原值**，换不换由你决定。
- **历史与登记不受影响**：分支点之前的历史消息逐字复制，分支来源、标题 `(N)` 序号、源会话内容都照旧；**不新增数据库列，不需要迁移**。
- **验证**：全量单测 **1651 例通过**（新增分支换 Agent 守门 21 例）；另有真实后端 + 无头 Edge 的端到端冒烟 26 项，含「新会话 Agent 下拉真的可用」这类可用性断言。
...
1. 下载 `cat-chat-2.8.1-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `cat-chat.exe`。
4. 在设置中添加在线 API 或本地模型服务。

> 归档里也提供旧名 `naiba-chat-2.8.1-beta-windows-x64.zip`（内含 `naiba-chat.exe`）——**两者内容等价，只是包内 exe 的文件名不同**，任选其一即可。安装后的文件名由你首次解压的那个决定，之后自动更新会一直沿用，不会中途改名。
...
- `cat-chat-2.8.1-beta-windows-x64.zip`
- `naiba-chat-2.8.1-beta-windows-x64.zip` —— 与上一个内容等价，仅包内 exe 名不同
...
这是 2.8.1 Beta，适合实际使用和反馈，但仍有以下边界：
...
$env:NAIBA_BUILD_VERSION = "2.8.1-beta"
