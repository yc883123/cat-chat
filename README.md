...
Cat Chat 是运行在 Windows 本机的通用 AI 自动化工作台。它把在线或本地模型、内置工具、后台任务、Skill、MCP、视觉工具和文件产物统一到一个对话界面中。2.8.2 Beta 修好了「端口被占用就白屏」这个老毛病，并把拖进输入区的文件夹变成一份路径清单——模型知道目录里有什么，而输入区只多出一个条目。
...
1. 下载 `cat-chat-2.8.2-beta-windows-x64.zip`。
2. 解压到一个可写目录。
3. 运行 `cat-chat.exe`。
4. 在设置中添加在线 API 或本地模型服务。

> 归档里也提供旧名 `naiba-chat-2.8.2-beta-windows-x64.zip`（内含 `naiba-chat.exe`）——**两者内容等价，只是包内 exe 的文件名不同**，任选其一即可。安装后的文件名由你首次解压的那个决定，之后自动更新会一直沿用，不会中途改名。
...
- `cat-chat-2.8.2-beta-windows-x64.zip`
- `naiba-chat-2.8.2-beta-windows-x64.zip` —— 与上一个内容等价，仅包内 exe 名不同
...
这是 2.8.2 Beta，适合实际使用和反馈，但仍有以下边界：
...
$env:NAIBA_BUILD_VERSION = "2.8.2-beta"
