...
发出去之后你会看到它**先去调一次接口验 Key**（一个「执行中」的工具块），通了才写进本机配置。
如果 Key 不对或没充值，它会把接口返回的原文贴给你，不会假装成功。
...
它不会闷头就提交：会先把**要提交的参数整理成一张表**念给你听（尺寸、模型、参考图挂在哪一格），
你点头之后才真的发出去。**表里哪一项不对，这时候改最省钱。**

AI 把参数整理好、提交到云端后，**在云端排队**。
